...
const ContentTeaser_central_1 = require("./data_objects/ContentTeaser_central");
const ParagraphContent_1 = require("./data_objects/ParagraphContent");
const ContentArticleDataTitleHx_1 = require("./data_objects/ContentArticleDataTitleHx");
const ContentStage_1 = require("./data_objects/ContentStage");
            ...
            switch (each_tag.type) {
                ...
                case 'tag':
                    if (/placement/i.test(cls)) {
                        ...
                        const txt_maxLen = 30;
                        const txt = $(each_tag).text().trim().replace(/[\n\r]+/, '');
                        console.log(`${colors.magenta(new Debug_1.Debug().shortInfo())} :: ${colors.red("Unknown #5")} :: type=[${each_tag.type}] name=[${each_tag.name}] id=[${tagObj.prop('id')}] class=[${$(each_tag).prop('class')}] text=[${txt.substr(0, txt_maxLen)}${txt.length > txt_maxLen ? "..." : ""}]`);
                    }
                    break;
                default:
                    {
                        const txt_maxLen = 30;
                        const txt = $(each_tag).text().trim().replace(/[\n\r]+/, '');
                        console.log(`${colors.magenta(new Debug_1.Debug().shortInfo())} :: ${colors.red("Unknown #6")} :: type=[${each_tag.type}] name=[${each_tag.name}] id=[${tagObj.prop('id')}] class=[${cls}] text=[${txt.substr(0, txt_maxLen)}${txt.length > txt_maxLen ? "..." : ""}]`);
                    }
                    break;
            }
        }
        ...
        for (let each_tag of tag.children) {
            ...
            switch (each_tag.type) {
                ...
                case 'tag':
                    if (/(module-video|form-collapsible|module-persona)/i.test(cls)) {
                    ...
                    else if (/(panel)/i.test(cls)) {
                    }
                    else if (typeof (tag_id) != 'undefined' && tag_id.trim().length > 0 && (typeof (cls) == 'undefined' || cls == null)) {
                    }
                        ...
                        result.push(ContentTeaser_central_1.ContentTeaser_central.init_central(url, $, each_tag));
                    }
                        ...
                        result.push(ContentArticleDataParagraph_1.ContentArticleDataParagraph.init(url, $, each_tag, false));
                    }
                    else if (/module-collapsible/i.test(cls)) {
                        this._parse_module_collapsible(url, $, each_tag, result);
                    }
                    else if (/h2/i.test(each_tag.name)) {
                        result.push(ContentArticleDataTitleHx_1.ContentArticleDataTitleHx.init_h2($, each_tag));
                    }
                    else if (/btn/i.test(cls)) {
                        result.push(ParagraphContent_1.ParagraphContent.initLink(url, $, each_tag));
                    }
                    else {
                        const txt_maxLen = 30;
                        const txt = $(each_tag).text().trim().replace(/[\n\r]+/, '');
                        console.log(`${colors.magenta(new Debug_1.Debug().shortInfo())} :: ${colors.red("Unknown #7")} :: type=[${each_tag.type}] id=[${tagObj.prop('id')}] name=[${each_tag.name}] class=[${$(each_tag).prop('class')}] text=[${txt.substr(0, txt_maxLen)}${txt.length > txt_maxLen ? "..." : ""}]`);
                    }
                    break;
                default:
                    {
                        const txt_maxLen = 30;
                        const txt = $(each_tag).text().trim().replace(/[\n\r]+/, '');
                        console.log(`${colors.magenta(new Debug_1.Debug().shortInfo())} :: ${colors.red("Unknown #8")} :: type=[${each_tag.type}] id=[${tagObj.prop('id')}] name=[${each_tag.name}] class=[${cls}] text=[${txt.substr(0, txt_maxLen)}${txt.length > txt_maxLen ? "..." : ""}]`);
                    }
                    break;
            }
        }
            ...
            switch (each_tag.type) {
                ...
                case 'tag':
                    if (/module-stage/i.test(cls)) {
                        result.push(ContentStage_1.ContentStage.init(url, $, each_tag));
                    }
                ...
                default:
                    {
                        const txt_maxLen = 30;
                        const txt = $(each_tag).text().trim().replace(/[\n\r]+/, '');
                        console.log(`${colors.magenta(new Debug_1.Debug().shortInfo())} :: ${colors.red("Unknown #19")} :: type=[${each_tag.type}] name=[${each_tag.name}] class=[${cls}] text=[${txt.substr(0, txt_maxLen)}${txt.length > txt_maxLen ? "..." : ""}]`);
                    }
                    break;
            }
        }
    }
    _parse_homeContent_pageGrid(url, $, tag, result) {
        for (let each_tag of tag.children) {
            const cls = $(each_tag).prop('class');
            const tagObj = $(each_tag);
            switch (each_tag.type) {
                ...
                case 'tag':
                    if (/home-col/i.test(cls)) {
                        ...
                        const txt_maxLen = 30;
                        const txt = $(each_tag).text().trim().replace(/[\n\r]+/, '');
                        console.log(`${colors.magenta(new Debug_1.Debug().shortInfo())} :: ${colors.red("Unknown #20")} :: type=[${each_tag.type}] name=[${each_tag.name}] class=[${cls}] text=[${txt.substr(0, txt_maxLen)}${txt.length > txt_maxLen ? "..." : ""}]`);
                    }
                    break;
                default:
                    {
                        const txt_maxLen = 30;
                        const txt = $(each_tag).text().trim().replace(/[\n\r]+/, '');
                        console.log(`${colors.magenta(new Debug_1.Debug().shortInfo())} :: ${colors.red("Unknown #21")} :: type=[${each_tag.type}] name=[${each_tag.name}] class=[${cls}] text=[${txt.substr(0, txt_maxLen)}${txt.length > txt_maxLen ? "..." : ""}]`);
                    }
                    break;
            }
        }
